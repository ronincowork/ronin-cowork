...
| `cowork_space` | **the cowork space**<!--g:glossary.cowork_space--> | The page with workspaces in it — the bar on top, workspace 1 and 2, the roster between them. Say *the cowork space*; not "the team workspace" (the team was its first tenant) and not *the coworkspace* (that is the whole UI). The map: `docs/cowork-space.md`. |
...
| `workspace_surface` | **surface**<!--g:glossary.surface--> | What a workspace holds: a terminal tile, the team commons, or the cowork commons (more coming). One per workspace, one head each. |
