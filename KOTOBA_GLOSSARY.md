...
| `message_queue` | **message queue**<!--g:glossary.message_queue--> | Inbound messages waiting to enter a particular live session. Delivered messages clear immediately; anything stuck remains visible under Messages for up to 48 hours. A missing target is refused; mail retained for a session that later ends shows **Target missing** and can never pass to a reused name. |
