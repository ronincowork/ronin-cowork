...
|---|---|---|
| `state.js` | 113 | DOM handle, constants, THEME, `tiles`, the shared-state object `S`, save/load |
| `errors.js` | 115 | `showFailure`, `guard`, `deadTile` — the containment layer |
| `viewport.js` | 15 | `setLayout` — its own module because three others need it |
| `api.js` | 36 | the `/api/sessions` calls |
| `widgets.js` | 128 | `makeDial`, `makeGauge` and the control-dial positions |
| `events.js` | 96 | the `/events` socket, birth/death chips, `openSessionSomewhere` |
| `home.js` | 114 | `refreshHome`, the catalog loaders, `showReceipt` |
| `commons.js` | 688 | `buildHome` — the commons panel and its tab strip (see `docs/commons.md`) |
| `wipeboard.js` | 298 | `buildWipeboard` — the ▤ Wipeboard pane |
| `projectroots.js` | 246 | `buildProjectRoots` — the ▣ Roots pane |
...
| `termview.js` | 169 | the 🔒 view — the untouched `tmux attach` xterm mirror, and touch drag-scroll |
...
| `tiledrop.js` | 180 | TOUCH ONLY — `collapseTileHead`, `makeDrop`: the one-row phone header |
| `tilemacros.js` | 144 | `buildTileMacros` — the ⚡ button on a tile head; prefills `+name: `, never runs |
| `voice.js` | 181 | dictation: `makeClipRecorder` + `wireDictation` (the 🎤 on the tile's compose box) |
| `panels.js` | 254 | `buildNotePanel` 📝, `buildTagPanel` 🏷, `toClipboard` |
| `macros.js` | 106 | `buildSessionPicker` — the pad key's session switcher |
| `pad.js` | 264 | keypad — bindings, chords, toasts, firing |
| `padpanel.js` | 500 | keypad — the ▦ panel and ask-on-press |
| `weblink.js` | 198 | keypad — WebHID programming of the device |
| `layout.js` | 418 | `build`, `buildDrawers` — assembling the page |
| `main.js` | 48 | `init` and the boot call |

The rule is nothing over ~700 lines; if a module passes that, split it rather than letting
it grow. **Nothing is in breach.** `tile.js` was, at 1,270, and was split on 2026-08-13 to
...
a ratchet). The client's equivalent is `check-modules` plus this paragraph — keep it true.
...
**3. Never reference an imported binding at module top level.** Everything cross-module
