...
import { createCampaignSurface } from './campaign.js';
...
export function createLeagueCommons(options) {
  ...
  const campaign = createCampaignSurface(), roster = createTeamRosterSurface(), templates = createTeamTemplatesSurface(options);
  const service = (el, enter) => ({ el, mount: () => {}, enter: () => enter?.(), leave: () => {}, destroy: () => {} });
  const services = { campaign: service(campaign.el, campaign.enter), roots: service(roots, rootRoom.enter), roster: service(roster.el, roster.render), templates: service(templates.el, templates.enter) };
  surface = WorkspaceKit.primitives.createChannelSurface({
    label: t('campaign.commons', 'Campaign commons'),
    channels: [
