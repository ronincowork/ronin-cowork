...
export function createTeamJikan() {
  ...
  whenInput.addEventListener('input', () => {
    ...
    previewTimer = setTimeout(async () => {
      ...
      whenPreview.textContent = r.ok
        ? t('team_jikan.when_preview', '{words} → next {next}', { words: r.data.words, next: (r.data.next || []).map(when).join(' · ') || t('team_jikan.never', 'never — that time has passed') })
        : r.message;
    }, 250);
  });
  ...
  form.addEventListener('submit', async (event) => {
    ...
    status.textContent = t('team_jikan.scheduled', 'scheduled — next {next}', { next: when(r.data.job?.next_run) });
  ...
  const outcomeWord = (job) => {
    if (!job.last_outcome) return t('team_jikan.not_yet', 'not yet');
    if (job.last_outcome === 'delivered') return t('team_jikan.delivered', 'delivered');
    if (job.last_outcome === 'queued') return t('team_jikan.queued', 'queued — waiting to enter');
    return job.last_outcome;
  };
  const row = (job, done) => {
    ...
    const timing = el('td', null, job.when_words || job.when);
    const next = el('td', null, done ? when(job.last_run) : `${when(job.next_run)}`);
    const last = el('td', null, done ? outcomeWord(job) : (job.last_run ? `${when(job.last_run)} · ${outcomeWord(job)}` : t('team_jikan.not_yet', 'not yet')));
    ...
    if (!done) {
      ...
      acts.append(action(t('team_jikan.run_now', 'Run now'), null, async () => {
        const r = await verb(job.id, 'POST', '/run');
        if (r.ok) note.textContent = t('team_jikan.ran', 'Ran {id}: {outcome}', { id: job.id, outcome: r.data.outcome?.outcome || '' });
      }));
    }
  ...
  const table = (heading, rows, done) => {
    ...
    const head = el('tr');
    for (const h of [t('team_jikan.col_request', 'Request'), t('team_jikan.col_to', 'To'), t('team_jikan.col_when', 'When'), done ? t('team_jikan.col_ran', 'Ran') : t('team_jikan.col_next', 'Next'), done ? t('team_jikan.col_outcome', 'Outcome') : t('team_jikan.col_last', 'Last'), '']) head.append(el('th', null, h));
    tbl.append(head);
  ...
  async function refresh() {
    ...
    try {
      ...
      note.textContent = t('team_jikan.help', 'A request delivered to one agent of this team, by name or to its lead, at a moment or on a rhythm. Ronin checks every minute and delivers through the message door — the dial is honoured, and a busy agent gets it queued. Nothing here starts an agent or a team.')
        + ' ' + (clock?.last_finished ? t('team_jikan.clock', 'Clock last checked {at}.', { at: when(clock.last_finished) }) : t('team_jikan.clock_waiting', 'Clock starting.'));
      paint();
    } finally {
      inFlight = false;
    }
