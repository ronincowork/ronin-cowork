# family_roles — WHO a session is, one file per role

> **DATA.** Nothing here executes. A **`family_role`** is the durable hat a session wears —
...
**Family**; `session_tasks` in every internal name, because a bare `family` is already the
...
`- **session_tasks:** A, B, C` is the whole link, and the role owns it — a task never names
...
`- **session_tasks:** —` is a role with an empty family, which is ordinary: it launches with
a blank task.
...
**Its reading is a shelf, not a field.** `role/<family_role>/` on the session-boot shelf is
