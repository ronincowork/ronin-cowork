...
- **lifecycle:** coding
- **ack:** no
- **opening:** Cut code from the plan doc: {prompt}. Work leg by leg, use scoped evidence while developing, and delete finished items from the doc. Commit coherent checkpoints privately; hand coherent work in to your team line with `tejun-desk hand-in` (the desks are in your brief — with none listed, commit to the repository's declared line instead). Never `git push` a desk and never run full BYOIN yourself: the lead's team promotion runs it once at team → dev under docs/test-protocols.md.
