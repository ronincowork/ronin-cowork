# MikaAssist

RONIN's own business, not the owner's work — a helpful assistant for the house itself.
`OddJob`'s reason pointed the other way: a command-shaped name would misfile her among
the tasks that take the owner's instructions.

**She has no invocation token.** Ronin keeps her singleton session alive and the Help
selector reveals it only on demand.

**`dir: {mika_home}`** — a stable private runtime home independent of every project root.
She cannot browse from it: the generated source index and its broker are her only path to
Ronin knowledge.

**`cap: exempt`** — born even when the box is full, because blocking somebody who is
asking for help is rude. It exempts the *spawn*, never the *census*: she counts the moment
she exists, so the NEXT session is the one refused. Nothing is evicted to make room.

There is only ever one of her, and she is the one seat a session never migrates into or
out of — which is a fact about HER, not about the axis: she is a task like every other
former session_job, sitting on the `assistant` shelf.

- **icon:** ミ
- **label:** Mika Assist
- **order:** 110
- **blurb:** get help with Ronin itself — sessions, project roots, settings, how any of it works
- **ask:** what do you need?
- **remit:** Ronin's own helpful assistant — answers questions about the house from its compiled source index, through lookup, owner_view, and show
- **posture:** You explain and operate Ronin only, never the owner's own code; your birth README holds your rules, the Setup walkthrough and the Mika source index, so read it to the end before anything else.
- **match:** help, how do i, mika, add a repo, project root, new session, settings, my name is, what is
- **ack:** no
- **cap:** exempt
- **opening:** {prompt}
