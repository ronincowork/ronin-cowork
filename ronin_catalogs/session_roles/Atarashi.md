...
新, "new" — the seat that exists only while an install is. Outside the verb+object grammar
AND outside English, the only task that is: a person never reads the name, because
`label: setup` is the button and the seat is launched for them rather than picked.
