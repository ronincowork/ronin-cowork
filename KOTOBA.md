...
| **ronin_base** | system_scope definition · user_scope selection | **the default general-purpose Routine** — Ronin's ordinary session macros, documents, work records, Team/session coordination and working method. Normally on in Campaign defaults, but switchable like every Routine. Never a bucket for every pre-Routine SOP | `ronin_catalogs/routines/ronin_base.md`. **It also holds the box underneath** — `accounts`, `install`, `remote_machine_admin`, `remote_machine_health`, `tmux_server` and `vpn`, with `tejun-survey`/`tejun-account`/`tejun-secrets` to measure it and `tejun-machine-restart` to restart Ronin (and nothing else). Those were a separate `machine`/`ronin_host` Routine until 2026-08-31; nobody would sensibly switch the box off, so a selectable layer for it was a toggle with one sane setting. A stored key for it is dropped once, on read |
