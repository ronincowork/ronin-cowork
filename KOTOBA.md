...
| **routine** [planned] | system_scope | **a named bundle of behaviours Ronin runs for you, with one switch** (owner, 2026-08-30): a reading list, the SOPs, the macros and the tools that together make one way of working possible — Ronin control (desks, hand-in, promotion), gbrain, Koshi, Hotwords, and future ones such as an agent-coordination methodology. The stock macro → action → SOP base is the core routine, always on. **Not a macro** (a routine may contain several), **not a `ronin_service`** (a service may back one), **not a session_role**. A switch applies to sessions born after it and touches nothing running. The owner sees them on the Settings page's **Routines** card; the bundle catalog and the birth-time wiring are the lab's `CONTROL_BUNDLES` build-out | `wip/buildouts/CONTROL_BUNDLES.md` (lab) |
