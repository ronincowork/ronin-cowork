...
| Term | Scope | Means | Record |
|---|---|---|---|
| **campaign** | user_scope | **the named body of work this Ronin configuration serves.** Today one install is one campaign: its project_roots, team_rosters and sessions are presented in that campaign's context. The campaign's name and description live in SETTEI. Joining a second campaign is future configuration switching, not a second meaning for `project_root` or `team_roster` | `src/user-config.ts` · `public/js/campaign.js` |
