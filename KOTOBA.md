...
| **test_protocols** | system_scope | the house’s testing arrangement, behind one provider-neutral pointer: who runs what, when. **Two audiences, two cadences** — ordinary Ronin development uses direct dogfood and scoped diagnosis, while one designated integrator runs the appropriate `byoin_check` mode once on the exact `dev → master` release candidate; a third-party install’s own agents run full BYOIN — `byoin_check`s plus `byoin_user_check` — after maintenance, an update, or any user-store customization (session role, skin, macro, SOP shadow). GitHub isolated checks run only on a PR to `master` or `workflow_dispatch`. Every agent-facing shelf README points here | `docs/test-protocols.md` |
...
| **desk_profile** | user_scope definition · session_scope selection | **the owner's standing defaults for the surfaces they work at** (R38): which **skin** (look — `SKINS.md`, tokens only, unchanged), which **lexicon** (the words a surface uses: *campaign* or *occasion*, *squad* or *staff*), which **campaign_kind** the board opens on, the **Team page's default arrangement**, the RIREKI **detail level** a tile shows — and more as surfaces earn defaults. **A desk_profile is NOT a skin; each desk_profile HAS a skin.** The house ships five — **terminal · vibe_code · professional · home · league** (owner, 2026-08-27 — League the gamer one, goofy on purpose); yours shadow stock. Chosen once, kept; not a step in any run | `concepts/ronin-league.html` (ronin-lab) lobby · `MANIFESTO.md` § 4 |
...
| **team_commons** | system_scope | **the team workbench's shared surface** — the wipeboard, the Docs slice and Team Configuration behind one tab strip on the team page. Named by the owner 2026-08-25, the `session_commons` word one level up: shared ground about the TEAM, inside its workbench. One of the three plug-and-play surfaces of the team workbench (terminal seat · roster · team_commons); the default RIGHT slot, with the lead's terminal left and the roster middle. The build-out is `wip/buildouts/TEAM_WORKBENCH.md` **[planned]** — today's channels surface is its seed | `public/js/team-view.js` |
| **cowork_space** | system_scope | **the page with workspaces in it** — a top header (the bar), two or four `workspace` cells (the 2×2 stacks 3 under 1 and 4 under 2; `count=2|4`), and a selector column left, centre or right. Named up a level on 2026-08-27 (owner: *"I've been calling it the team workspace. But to be honest, this is the cowork_space"*): the team was its first tenant, not its definition — the League and others are coming to it. `#/team/:name` is one address into it. Not the `coworkspace` (the whole UI); this is one view of it. R38's desk_profile applies its "default arrangement" HERE. **The map is `docs/cowork-space.md`** — one document to point at | `docs/cowork-space.md` · `public/js/team-view.js` |
| **workspace** | system_scope | one SLOT of the cowork_space, holding exactly one `workspace_surface` at a time (`workbench.place`) and remembering which per tab. Say *workspace 1*, *workspace 2*. A workspace is a place, never a surface; the Kit's layout map shows, hides and reorders them | `public/js/team-view.js` |
| **workspace_surface** | system_scope | the genus: what a workspace holds. **Three today, peers** — `terminal_tile` · `team_commons` · `cowork_commons` — with *league* and others planned. One surface per workspace, one head per surface, traded by `place()`; a surface never draws over another (the `admin_desk` overlay is the one exception, and it retires when `cowork_commons` lands). Alias: **surface**, when the context is the cowork_space | `docs/cowork-space.md` |
...
| **cowork_commons** | system_scope | **the install's shared surface** — this install and this owner, behind one tab strip: Machine health · Account (the desk's rail: Configuration · Appearance · Release & update · Hotwords · Koshi · gbrain · Log out) · Desk profile · Project roots · Roster · Archived (the tile commons' two, moved here 2026-08-27) · Help desk · Keypad. The `admin_desk` re-hung as a `workspace_surface`, a peer of `team_commons` (owner, 2026-08-27: *"too many things in one thing … let's call it a cowork_commons"*). The `session_commons` word two levels up: shared ground about the INSTALL, in a workspace. Never bare "the commons" (tile-scoped, about sessions); never "the admin desk" once it lands. Landed 2026-08-27 (`public/js/cowork-commons.js`); build-out record: ronin-lab `wip/buildouts/COWORK_COMMONS.md` | `docs/cowork-space.md` · `public/js/cowork-commons.js` |
| **selector_column** | system_scope | a column of the cowork_space that PICKS what goes into a workspace and never holds a surface itself. The **roster** is the one there is — member cards, click seats one in a workspace, the 人 pinned in workspace 1 | `public/js/team-view.js` |
