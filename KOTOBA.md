...
| **job_class** | user_scope | a shelf the owner invents to organize the ＋ New board's session_jobs — drawn as a collapsible grouping, NAMED class by ruling (owner, 2026-08-21: *"we should not be using groups"*): `group` is the roster's addressing word (`+tag:` resolves to live members) and a job_class addresses NOTHING — no API resolves one, no behavior branches on one. A job may sit on several shelves; membership lives in the JOB_CLASSES.md side manifest in the catalogs store, never in the shipped catalog, so an upgrade cannot clobber a shelf and a shelf cannot pin a house job to a stale entry | `src/catalog.ts` (JOB_CLASSES) |
