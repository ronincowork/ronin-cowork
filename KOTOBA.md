...
| **JIKAN** | system_scope clock · user_scope list | **時間, "time" — the house's one clock, and the Cron jobs on it. The rails for everything timed** (owner, 2026-09-03): the message queue's retry, the sessions broadcast, the Tomodachi sweep and the scheduled requests are each a tick on the one clock (`onClock`) — never two runs of one tick at once, a throw recorded on the face and never escaping, one boot-run rule, one `clockFace()` that says what is ticking; nothing else in `src/` sets a rhythm, so one fix fixes all. **Cron jobs** is the owner's word for the requests on it. A job is one request (the words delivered), addressed to one session of an ACTIVE team by name or to whoever leads it (`to: lead`), at a moment (`once 2026-09-04 08:00`) or on a rhythm (`daily 08:00`, `weekdays 08:00`, `weekly mon 08:00`, `monthly 1 09:00`, `hourly`, `every 30m`, a five-field cron line). The always-on server ticks once a minute and delivers what is due through the ordinary message door (`message_queue`, source `jikan`), so the dial is honoured and a busy session gets it queued. **It births nothing** (owner, 2026-09-03): a job whose session is gone is refused and left standing. The list is the owner's — one `.md` per team in the `jikan` store, `## id` + `- **key:** value`, hand-editable; `state: paused` holds, `done` is the clock's alone; a missed moment fires once, never a backlog. Surfaces: the Cron jobs tab on the team commons; `tejun-jikan` for an Agent (a `schedule-request` is a propose-and-confirm) | `src/jikan.ts` · `src/routes/jikan-api.ts` · `public/js/team-jikan.js` · `ronin_bin/tejun-jikan` · `docs/jikan.md` |
