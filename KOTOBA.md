...
| **required reader** | system_scope | **the definition reaping turns on**: a post's addressees if it names any, otherwise every live member. A `--to none` post has none, so only the TTL retires it | `src/wipeboards.ts` |
| **reap** | system_scope | the machine retiring a post — every required reader has read it and the grace has passed, or it aged past the TTL. Runs inline on every check and post: no daemon, no timer. **The reaper is the only deleter**; no button and no agent deletes a post, and a wipeboard is removed whole only when nothing points at it any more | `src/wipeboards.ts` |
