...
| **cowork_commons** | system_scope | **the install's shared surface** — this install and this owner, behind one tab strip: Machine health · Account · Desk profile · Project roots · Help desk · Keypad. The `admin_desk` re-hung as a `workspace_surface`, a peer of `team_commons` (owner, 2026-08-27: *"too many things in one thing … let's call it a cowork_commons"*). The `session_commons` word two levels up: shared ground about the INSTALL, in a workspace. Never bare "the commons" (tile-scoped, about sessions); never "the admin desk" once it lands. Build-out: ronin-lab `wip/buildouts/COWORK_COMMONS.md` **[planned]** | `docs/cowork-space.md` |
