...
| **routine_floor** | system_scope | **the mandatory machinery that resolves, delivers and records Routines; not itself a Routine and never a switch** — unified launch, Campaign/Team resolution, universal vocabulary and shelf map, minimum command delivery, Control initialization and the birth receipt. Kept deliberately small: ordinary fork, tell, wipeboard and document behaviours belong to `ronin_base`, not the floor | `docs/routines.md` |
