...
**Our internal system names stay ours.** Twenty, and this is the list — `KOTOBA_GLOSSARY.md`
repeats it verbatim and may not carry a twenty-first this file does not have:

> **TEJUN · MICHI · TEGAMI · SHINGO · RIREKI · OBOERU · TOMODACHI · SOROBAN · KOSHI · KOE ·
> DAIKUSAN · KOTOBA · AGERU · JUSHO · BYOIN · SETTEI · KYOKAI · SHIWAKE · KOKUGO · JIKAN**
...
| **test_protocols** | system_scope | the house’s testing arrangement, behind one provider-neutral pointer: who runs what, when. **Two audiences, two cadences** — ordinary Ronin development uses scoped diagnosis; team promotion runs the first full repository BYOIN on the exact `team/<team>/dev → dev` candidate, and `dev → master` CI consumes that receipt then runs the second full repository BYOIN on the exact release candidate (owner, 2026-09-01). A third-party install’s own agents run full BYOIN — `byoin_check`s plus `byoin_user_check` — after maintenance, an update, or any user-store customization (session role, skin, macro, SOP shadow). Every agent-facing shelf README points here | `docs/test-protocols.md` |
| **commit_train** | dev_scope | **the accepted hand-ins standing on a team line between promotions; a promotion takes the whole commit_train — nothing boards half-reviewed** (owner, 2026-09-01). Internal working vocabulary, never user-facing; *train* is the spoken alias only, never the record spelling. **The commit_train rule** (alias: the same-train rule): two changes that would break the line landing apart — a door and its callers, a record and its form — ride one commit_train or none | `docs/test-protocols.md` |
...
| **ronin_worktrees** | system_scope definition · user_scope selection | **the managed-worktree Routine** — managed worktrees, desk reading, hand-in, lead integration, receipts, Git safeguards and their tools. Selecting it additively includes `ronin_base`; it does not require optional `ronin_services`. It coordinates parallel file work; it does not claim authority over Agents. A repository arrangement says where its worktree behaviour applies; it is not a second Routine switch. **Named `ronin_control` until 2026-08-31**, when `control` was carrying five separate meanings: the session dial (`@ronin-control`), this Routine, the Settei bundle labelled Ronin Control, the Routine class enum, and `ronin_control_surface`. The dial is the word&rsquo;s oldest and most load-bearing owner, so the Routine took the word its own definition already used. Pre-rename records are not migrated: this is MVP, and the only box holding the old key had already been carried before the compat table was removed | `ronin_catalogs/routines/ronin_worktrees.md` · `docs/control-surface.md` |
...
| **ronin_host** | system_scope definition · user_scope selection | **the Routine about the box, not about Ronin** — the machine an install sits on: `accounts`, `install`, `remote_machine_admin`, `remote_machine_health`, `tmux_server` and `vpn` as discoverable SOPs, two short injected readings, and `tejun-survey`/`tejun-account`/`tejun-secrets` to measure rather than assert. That is the line against `ronin_services`, which is Ronin OWN optional machinery; the host would exist whether or not Ronin were on it. An ordinary selectable Routine: it requires `ronin_base` like its siblings and is popped on and off in the picker. It reports and diagnoses and repairs nothing unasked — the session engine especially is never a session to restart, since it owns every session and the repair would end the Agent doing it. The one restart an Agent may perform is Ronin own, and `tejun-machine-restart` lives HERE: an Agent without the host tools has no business restarting anything. Nothing is lost by that placement, because the guard shims are `routine_floor` and reach every Agent regardless of Routine, so a session without this Routine that reaches for `systemctl` is refused rather than left to improvise. **Named `machine` until 2026-08-31**, a word already spent on `ronin_machine`, `setup.machine_name` and `campaign.machine` | `docs/routines.md` · `ronin_catalogs/routines/ronin_host.md` |
...
| **JIKAN** | system_scope clock · user_scope list | **時間, "time" — the house's one clock, and the Cron jobs on it.** Every timed thing in the server is a tick on the clock (`onClock`): the message queue's retry, the sessions broadcast, the Tomodachi sweep, the Cron jobs — an interval that never overlaps itself and never throws out; nothing else sets a rhythm, so one fix fixes all. **Cron jobs** is the owner's word for the requests on it, and a job is a ping, not infrastructure (owner, 2026-09-03): one request, one session of an active team by name or `lead`, one `due` date; every minute, anything due at or before now goes through the message door and is marked done (once) or given its next due (repeat); a missed beat is missed; *run at next tick* sets `due` to now. Births nothing. One `.md` per team in the `jikan` store, the owner's, hand-editable | `src/jikan.ts` · `public/js/team-jikan.js` · `ronin_bin/tejun-jikan` · `docs/jikan.md` |
...
| **template_library** | system_scope · ronin_service | **the shelf of template bundles Ronin keeps and grows — a Ronin Services feature** (owner, 2026-09-03). Served by HQ (`ronin-library/1`: one card per bundle — face, `url`, `sha256`, `holds`) to an entitled box, read through the one AGERU client with the Services token on a press of the Campaign page's Templates card, never on a timer; everything a bundle holds and the plan are shown before anything is written. ronincowork.com shows descriptions only. Without Services the shelf is opaque and the card names the switch; the handful of templates that ship inside Ronin are the floor, and making your own always is | `src/routes/library-api.ts` · `public/js/campaign-templates.js` |
...
| **`gbrain_disconnected`** | system_scope | the launch-table key holding a provider's declared tokens for a `gbrain_mode: disconnected` launch. DATA, never a code path: Ronin names the intent and the provider section names its CLI spelling. OpenAI disables gbrain specifically; Anthropic's available control is coarse and disables **ALL MCP** for that launch. A requested disconnect with no declaration is refused; a profile default that the provider cannot deliver degrades to connected and the receipt says so | `ronin_catalogs/PROJECT_ROOTS.md` · `src/spawn.ts` |
...
| **agent_defaults** | user_scope | the Campaign or Team bucket holding values that seed an Agent form: `provider`, `model`, `reach`, `recruit`, `output`, `dial`, `launch_mode`, `gbrain_mode` | `CASCADE.md` (ronin-lab) § 4 · `NEW_AGENT.md` § 7.2 |
...
| **output** | user_scope · session_scope | the mandate list for what the Agent hands back, including explicit negative instructions such as `no code`; accepts legacy single strings by wrapping them, and validates tokens but never their combination | `NEW_AGENT.md` (ronin-lab) R-a · § 7 |
| **launch_mode** | user_scope · session_scope | the additive provider-command choice that cascades Campaign → Team → Agent: `configured` leaves the provider command unchanged and therefore preserves whatever configuration its CLI loads; `live_dangerously` appends that provider's declared bypass flag for this launch. Stock Campaign value: `live_dangerously`. A requested `live_dangerously` with no provider declaration is refused, never silently treated as `configured`. User-facing choices: **Model provider configuration** · **Dangerously** | `ronin_catalogs/PROJECT_ROOTS.md` · `src/spawn.ts` · `CASCADE.md` (ronin-lab) § 1–§ 4 |
| **gbrain_mode** | user_scope · session_scope | the gbrain-reach choice that cascades Campaign → Team → Agent: `connected` appends nothing and leaves the provider's loaded MCP configuration standing; `disconnected` appends its declared `gbrain_disconnected` tokens. Stock Campaign value: `disconnected`. A requested disconnect with no declaration is refused. **On Anthropic, disconnected disables ALL MCP for that launch, not only gbrain**; the provider has no narrower declared launch control | `ronin_catalogs/PROJECT_ROOTS.md` · `src/spawn.ts` |
...
| **message_queue** | system_scope | the live flow of inbound messages waiting to enter sessions — a transport, never a record. A send to a nonexistent name is refused with roster/wipeboard teaching. Accepted mail binds `target_key` to the recipient session's durable birth identity, never its reusable name; retained mail carries `expires_at` and is reaped after 48 hours. Successful deliveries disappear immediately; blocked, failed, or `target_missing` deliveries stay visible in Team Commons, and a later session with the same name never receives them. It never contains wipeboard posts, only the separate inbound notice a post may generate | `docs/message-queue.md` |
| **queued_message** | session_scope | one inbound message waiting to enter one named session: direct tell, wipeboard notice, owner send or house notice. Carries explicit `from` and `target` addresses; durable until delivered or dismissed | `docs/message-queue.md` |
