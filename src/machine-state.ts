 ...
 *
 *     machine_settings.json                    what the owner typed  (only the UI writes it)
 *         |
 ...
 * property: `machine_settings.json` also carries `auth` — a scrypt record and the secret that signs
...
/**
 * THE CAMPAIGN AND THE DESK PROFILE LIVE IN `machine settings campaign record` NOW, NOT HERE.
 *
 * `doc.campaign` (name, description) and `doc.desk` (profile) were this file's until
 * 2026-08-29. They describe a BODY OF WORK, and `machine_settings.json` is the record of a MACHINE —
 * so when a Campaign became a durable object of its own they moved to its store, and
 * `src/campaigns.ts` is the one writer of both. Its `readCampaignSection` /
 ...
 * when `machine_settings.json` does not exist, which is the same signal the project-root floor
 ...
 * with nothing reading it. The owner edits `machine_settings.json`; there is no writer here until
