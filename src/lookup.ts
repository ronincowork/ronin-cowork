...
export async function expandLookup(text: string): Promise<string | null> {
  ...
  // `+wipeboard: <name>` — same contract as `+tag:`: the answer, not an errand. What
  ...
  const wb = /^\s*[+ろ/]?(wipeboards?|boards?)\s*:?\s*([a-z0-9_-]*)\s*$/i.exec(text);
  if (wb) {
    const want = wb[2].toLowerCase();
    if (!want) {
      const names = new Set(await listBoardFiles());
      const counts = new Map<string, number>();
      for (const s of all) for (const b of await getWipeboards(s.name)) { names.add(b); counts.set(b, (counts.get(b) ?? 0) + 1); }
      if (!names.size) return `${text.trim()} → no wipeboard exists yet. The owner starts one in Ronin's ▤ Wipeboard tab. Nothing to look up.`;
      const lines = [...names].sort().map((b) => `${b} (${counts.get(b) ?? 0})`);
    ...
    if (!isValidBoardName(want) || !(await boardExists(want))) {
      const known = [...new Set(await listBoardFiles())].sort();
      return `${text.trim()} → there is no wipeboard "${want}".` + (known.length ? ` Boards that exist: ${known.join(', ')}.` : ' No board exists yet.');
    }
    const board = await readBoard(want);
    const rows: string[] = [];
    for (const s of all) {
      if ((await getWipeboards(s.name)).includes(want)) {
        rows.push(`${s.name} [${DIAL_ICON[await getControl(s.name)] ?? '🤖 write'}]`);
      }
    }
    const brief = board.brief.replace(/\s+/g, ' ').trim().replace(/\.$/, '');
    return (
      `${text.trim()} → resolved by Ronin (no lookup needed): wipeboard "${want}" is ${boardPath(want)}. ` +
      `Brief: ${brief || '(empty — the owner has not written one yet)'}. ` +
      `On it: ${rows.length ? rows.join(', ') : 'nobody yet'}. ${board.posts.length} post(s) so far. ` +
  ...
  // WHOLE message is the invocation ("+tag: ronin") — the message becomes the answer.
  const solo = /^\s*[+ろ/]?(tags?|groups?)\s*:?\s*([a-z0-9_-]*)\s*$/i.exec(text);
  if (solo) {
    const want = solo[2].toLowerCase();
    if (!want) {
      const counts = new Map<string, number>();
      for (const s of all) for (const t of s.tags) counts.set(t, (counts.get(t) ?? 0) + 1);
      if (!counts.size) return `${text.trim()} → no session carries a tag yet. Groups are set by the owner (🏷 on the tile header in Ronin). Nothing to look up.`;
      const lines = [...counts.entries()].sort().map(([g, n]) => `${g} (${n})`);
      return `${text.trim()} → groups in play, resolved by Ronin (no lookup needed): ${lines.join(', ')}. Ask for one by name — "+tag: <group>" — to see its members.`;
    }
    const rows = await rosterOf(want);
    if (!rows.length) {
      const known = [...new Set(all.flatMap((s) => s.tags))].sort();
      return (
        `${text.trim()} → nothing is tagged "${want}".` +
        (known.length ? ` Groups that exist: ${known.join(', ')}. Don't assume a near-match — "kojin" and "kojinsa" are different groups.` : ' No group exists yet.')
      );
    }
    return (
      `${text.trim()} → resolved by Ronin (no lookup needed): the "${want}" group is ${rows.length} session(s) — ${rows.join(', ')}. ` +
      `That set is what "${want}" means here. Membership changes as sessions are born, tagged or die, so re-run "tejun-group ${want}" before acting on the group later, and control-check each member before touching it. ` +
      `This is a lookup: report it and wait unless you were already told what to do with them.`
    );
  }

  // INLINE — "summarize what's happening across +tag: ronin". The owner's sentence is
  ...
  // The leading + (or /) is REQUIRED here: without it, prose like "tag: the release"
  // would be mistaken for an invocation.
  const inline = [...text.matchAll(/[+ろ/](tags?|groups?)\s*:?\s*([a-z0-9_-]+)/gi)];
  ...
  for (const m of inline) {
    ...
    clauses.push(
      rows.length
        ? `+tag: ${want} = ${rows.length} session(s) — ${rows.join(', ')}`
        : `+tag: ${want} = NO SUCH GROUP (nothing is tagged "${want}"; don't guess a near-match)`,
    );
  }
  return (
    `${text.trim()}  ⟨resolved by Ronin, no lookup needed: ${clauses.join('; ')}. ` +
    `Membership can change — re-check with "tejun-group <group>" before acting later, and control-check each member before touching it.⟩`
  );
}
