...
export async function closeAssignedDesks(
  ...
  for (const desk of desks) {
    ...
    if (desk.pending) {
      ...
      actions.push(`run worktree-desk sync ${id(desk)}`);
    }
    if (desk.blocked) {
      reasons.push(`blocked: ${desk.blocked}`);
      actions.push(`run worktree-desk status ${id(desk)} and use worktree-desk reply ${desk.repo} <receipt-id> <message> to resolve it with the lead`);
    }
    if (desk.ahead > 0) {
