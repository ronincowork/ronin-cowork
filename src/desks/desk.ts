 ...
 * behaviour.
 */
export async function closeDeskForShutdown(repo: string, branch: string, session: string, runtime: CloseRuntime = liveRuntime): Promise<CloseOutcome> {
  return closeDeskWithOptions(repo, branch, runtime, session, false);
}
