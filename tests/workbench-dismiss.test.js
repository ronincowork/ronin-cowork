...
test('New Team is consumed only after complete creation and destination opening', async () => {
  ...
  assert.match(form, /reset\(\);\s*openLaunchHandoff\(\{ team: name, sessions: launched \}, launchTab\);\s*await created\?\.\(name\);\s*await consumed\?\.\(\);/);
  const partial = form.slice(form.indexOf('if (refused.length)'), form.indexOf("notice.set('', '')"));
  assert.doesNotMatch(partial, /consumed/);
  assert.match(partial, /openLaunchHandoff\(\{ team: name, sessions: launched \}, launchTab\);\s*return;/,
    'a failed cast opens the Team destination even when no Agent launched');
