 ...
 *
 *   every former session_job  →  a session_role, all eleven, without exception
 *   every former shelf        →  a role_family, with the same family it always had
 *
...
test('every former session_job is a session_role — all eleven, and nothing extra', async () => {
...
test('the three shipped shelves are the three stock role_families, with their families intact', async () => {
  ...
  for (const r of roles) {
    // SET equality: membership is the frozen history. ORDER is presentation now — the
    // reader pins a family's default_lead_role first (R35), so developer comes back
    // QuarterBack-first while its membership stays the shelf it always was.
    assert.deepEqual(
      [...r.session_roles].sort(),
      [...FORMER_SHELVES[r.name]].sort(),
      `${r.name}'s family must be the shelf it always was`,
    );
    if (r.default_lead_role) {
      assert.equal(
        r.session_roles[0],
        r.default_lead_role,
        `${r.name}'s default_lead_role is pinned first on its shelf`,
      );
    }
  }
});
...
test('every task is on exactly one stock shelf — none loose, none shelved twice', async () => {
  ...
  for (const r of await listRoleFamilies()) {
    for (const t of r.session_roles) placements.set(t, (placements.get(t) ?? 0) + 1);
  }
...
test('no former job was promoted to a role, and no shelf was demoted to a task', async () => {
  ...
  const roles = (await listRoleFamilies()).map((r) => r.name);
  const tasks = (await listSessionRoles()).map((t) => t.name);
  for (const job of FORMER_JOBS) {
    assert.ok(tasks.includes(job), `${job} was a session_job, so it must be a session_role`);
    assert.ok(!roles.includes(job.toLowerCase()), `${job} must not have become a role_family`);
  }
  for (const shelf of Object.keys(FORMER_SHELVES)) {
    assert.ok(roles.includes(shelf), `${shelf} was a shelf, so it must be a role_family`);
    assert.ok(!tasks.includes(shelf), `${shelf} must not have become a session_role`);
  }
});
