...
test('consumers cannot override the shared hidden detail or stone geometry', async () => {
  ...
  assert.match(css, /@container stone-work-surface-seat \(min-width: 64rem\) \{\s*\.wk-surface:not\(\[data-flush='true'\]\) > \.wk-surface-content\.sws-host \{ padding-inline: calc\(var\(--space-12\) \* 2\); \}/);
