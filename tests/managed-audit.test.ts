...
const cleanRepo = (): ManagedRepositoryObservation => ({
  repo: 'cowork', mode: 'managed', dev_tip: 'dev', team_tip: 'line', managed_paths: ['/w/live'], live_sessions_from_registry: ['live'], publish_refs: ['dev', 'master'],
  ...
  release: { dev_sha: 'dev', stable_sha: 'old', working: 'dev', stable: 'master', open_prs: [{ base: 'master', head: 'dev', state: 'open' }] },
});
...
test('fixture classifies all seven managed invariants and reports lag separately', () => {
  ...
  repo.managed_paths = [];
  repo.live_sessions_from_registry = [];
...
test('current construction catches a non-current base when distance cannot explain it', () => {
  ...
  repo.refs.push({ name: 'candidate/old', sha: 'candidate', kind: 'candidate', contained_in_dev: false, base_sha: 'old-dev' });
...
test('rolling release state requires exactly one matching open PR', () => {
  ...
  const result = auditManagedState({ ledger, projection, repositories: [repo] });
  assert(result.findings.some((item) => item.code === 'multiple_release_prs'));
});
...
test('user-managed checkout Git is excluded even when it resembles managed residue', () => {
  ...
  checkout.live_sessions_from_registry = [];
  checkout.refs[0] = { ...checkout.refs[0]!, remote: true };
  checkout.release.open_prs = [];
