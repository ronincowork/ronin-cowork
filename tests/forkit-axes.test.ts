 ...
 * a fork carried a blank `job_role` for its entire life — and because the role is stamped
...
test('forkit resolves both launch axes, and says how each defaults', async () => {
  const text = await forkit();
  assert.match(text, /`job_role`/, 'the role is a parameter');
  ...
  assert.match(text, /job_role` is INHERITED from the origin/i);
  ...
  assert.match(text, /resolved `job_role` \+ `session_task`/);
});
...
test('forkit reuses the canonical launch contract and gets the whole Build Brief', async () => {
  ...
  assert.match(text, /all-session reading \+ the project_root's \+ the\s+job_role's \+ the session_task's/);
...
test('a fork owns its task afterwards, and never its role', async () => {
  ...
  assert.match(text, /`job_role` does not move with it, and nothing can change it/);
});
