...
const campaign = {
  ...
  config: { agent_defaults: agentDefaults({
    ...
    routines: { ronin_base: true, machine: true }, behaviours: ['ways:plan'], dial: 'write', permissions: 'default',
  }) },
} as CampaignConfig;
const team = {
...
const sources = (roster: TeamRoster | null) => ({
  ...
  routines: [routine('ronin_base'), routine('machine')],
  desk: 'own' as const,
});
...
test('rōnin seed reads Campaign and reports the frozen residue', () => {
  ...
  assert.equal(seed.routines.find((r) => r.name === 'machine')?.on, true);
});
...
test('Team seed reads its complete map with no live Campaign inherit', () => {
  ...
  assert.equal(seed.routines.find((r) => r.name === 'machine')?.on, false);
});
