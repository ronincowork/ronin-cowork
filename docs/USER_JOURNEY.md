...
**Current state:** live at [www.ronincowork.com](https://www.ronincowork.com/) from the
`ronin-site` repository on Azure.
The public supporting pages explain Tailscale private access and tmux-backed persistent agent
sessions without adding stops to the five-surface journey.
...
**Current state:** live at the
[Load Ronin page](https://www.ronincowork.com/load-ronin.html). The one canonical command is:
...
machine and agent facts, and keeps a live **When you save** review beside the form. The old
`?setup` and `?cowork_setup` spellings exist only as compatibility redirects. The Shiwake activation contract
above remains required; a form that only stores the email locally is a regression.
