...
| **historical / generated / incidental** | commit hashes in diaries, `git grep` in check scripts, "push" as a verb about events or palettes, lexicon tables, `branch` as a code path | `scripts/check-*.mjs`, `scripts/kokugo-table.mjs`, `docs/kokugo-table.md`, `ronin_catalogs/lexicons/professional_en.md`, `docs/ui.md`, `docs/ui-agents.md`, `docs/model-providers.md`, `docs/cowork-space.md`, `src/ws/*`, `src/sockets.ts`, `src/machine-settings.ts`, `public/style.css`, the rest of `public/js/*`, `tests/*`, `wip/buildouts/*`, `KOTOBA.md`, `KOTOBA_GLOSSARY.md`, `README.md` | no session instruction; nothing to migrate |
