...
- `ronin_bin/write_tegami` validates `repos[]` entries as exactly `{repo, branch}`; when
  track 4's TEGAMI entry carries desk fields the validator must widen or the letter refuses
  its own state.
