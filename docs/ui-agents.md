...
dark theme from the answer. Once it has, it paints in **truecolor**: `48;2;240;238;232`
(#f0eee8) for its input box, `38;2;31;35;40` for text, `48;2;218;251;225` for an added diff
line. None of that is reachable by any palette, so the choice is final for the life of the
process.

Two sessions on this box prove it, same CLI, different startup:

| session | started | what it emits |
|---|---|---|
| `settei_rollout` | before the terminal followed the shell | **0 truecolor**, 5181 cube codes |
| `roster` | after | **6058 truecolor**, a light theme |

`roster` started while the shell was light, was told "light", and stayed light — a cream
input box glaring out of a dark tile, immune to flipping back.
