...
|---|---|---|---|
| Machine and Campaign settings, installation choices, Setup answers, provider facts, defaults | [machine-settings.ts](../src/machine-settings.ts); [campaigns.ts](../src/campaigns.ts) uses its Campaign section | [machine-settings-api.ts](../src/routes/machine-settings-api.ts), [campaigns-api.ts](../src/routes/campaigns-api.ts) | Setup answers are explicit person intent (`acted` or `not_now`) per step; provider observations and other readiness are separate measured facts; schema and migration belong with the settings writer |
...
| Browser preferences, drafts, and Workbench launches | Relevant client module's browser storage, e.g. [state.js](../public/js/state.js); [workspace.js](../public/js/workspace.js) owns per-tab Workbench state and consumes structured launch URLs once | UI only | Presentation persistence; launch instructions are removed from the URL before use, then ordinary per-tab state owns refresh; never authority for server operations |
