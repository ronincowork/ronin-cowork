...
| Browser preferences, drafts, and Workbench launches | Relevant client module's browser storage, e.g. [state.js](../public/js/state.js); [workspace.js](../public/js/workspace.js) owns per-tab Workbench state and short-lived launch tokens | UI only | Presentation persistence; launch payloads are claimed and deleted once, then ordinary per-tab state owns refresh; never authority for server operations |
