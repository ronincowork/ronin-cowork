...
tmux server, keeping Cowork egress narrow and visible, and making the install removable.
...
| open-source Cowork license | `head -3 LICENSE` and `test -f NOTICE` | Apache License 2.0 and NOTICE |
...
| actual listening address | `ss -ltnp | grep -i node` | loopback or the machine's tailnet address; investigate any wider bind |
| websocket origin check | `grep -n originAllowed src/index.ts src/ws/origin.ts` | a page Ronin did not serve is refused |
| Cowork egress | `grep -RIn 'ALLOWED_HOST\|EgressRefused\|fetch(' src` | activation transport is allowlisted; investigate every other call site |
| actual egress record | `cat "$(bin/ronin-store session)/../egress.jsonl"` | absent/empty on fresh Cowork, or entries the owner can account for |
...
- **Network:** with no Ronin password, the tailnet is the wall. Everyone on it can use the
  shell. Loopback plus an SSH tunnel is also valid. Never expose the port publicly.
...
- **Provider data:** the Agent CLI talks directly to its provider under the owner's account.
  ...
  model name.
- **Cowork egress:** installation uses GitHub. Cowork does not proxy Agent traffic. Optional
