...
proved the PR's head commit (`docs/development/release.md`). Do not push per-feature, per-agent, phase,
...
(`ronin_catalogs/behaviours/conditional/worktree-root.md`; the model is `docs/architecture/worktrees.md`). Commit preserves work privately at the desk;
...
published to the remote and never opened as a PR. In a checkout (`ronin_catalogs/behaviours/conditional/checkout.md`), the
...
roster nests its storage where a root does not, is `docs/architecture/campaign-scope.md`.

## Implementation authority

- Catalog storage and validation: `src/project-roots.ts`
- Campaign scoping, the refusals and the migration: `src/campaign-scope.ts` · `docs/architecture/campaign-scope.md`
...
- Session-boot shelf: `docs/architecture/session-boot.md`
