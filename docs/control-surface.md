...
## Opting out is a setting, not a departure — 2026-08-29

The owner's objection, verbatim in spirit: *"opening a terminal in a tile and launching a
session loses most or all edge management — that's binary. Is there a SETTEI setting for
the control surface on/off, changed by including or not including reading material in
buildBrief? Better, clear instructions pointing people to Customize, with shadow SOPs."*
Yes, and every seam it needs already exists. The design:

### One leaf, four scopes, most specific wins

`control_surface` is one SETTEI leaf (`src/settei-registry.ts`, section *Working*,
family `control_surface`), answered per read like every other leaf and carrying its
provenance, so the ⚙ tab can always say *why* a session did or did not get a desk:

| Scope | Where it is set | Values | Today |
|---|---|---|---|
| install | `ronin.json` → `control_surface.mode`, written by `PUT /api/settei/control_surface` | `on` · `off` | `RONIN_DESKS` env (cutover switch); becomes the override, the leaf becomes the truth |
| repository | `RONIN_REPO` `desks=` | `managed` · `none` | exists |
| team | roster `desks` | `on` · `off` | to add — a team of writers on a code repo needs none |
| session | the launch box's one control | `own` · `none` · absent | exists (`DeskChoice`) |

Resolution: session → team → repository → install; the first explicit answer wins; absent
everywhere = by lifecycle (`DESK_LIFECYCLES`). `off` at any scope is honest: no desk, no
desk words, the repository's declared line as its own instructions say. The one thing no
scope may do is produce a half-state — a desk with no `hand_in` in the brief, or desk
words to a session standing in `dev` — because *mixed instructions are worse than none*.

### The brief follows the setting — the MCP precedent

`buildBrief` already does exactly this for MCP (owner's ruling, 2026-08-17): a session
launched with MCP off reads no `*_connected` shelf; the tools and the reading about them
ride the same choice. The control surface rides the same way:

- **on** → the desk block, `ronin_session_boot/assignment/DESK_CONTRACT.md` in *Read
  first*, the desk actions in the catalog the session sees, `tejun-desk` on its PATH.
- **off** → none of that; instead the shared-checkout reading — `ronin_sops/github.md`'s
  direct/shared mode, the claim guard's own explanation — and the `cut-code` / `land-work`
  / `check-clean` entries in their pre-desk form. The session is told the arrangement it
  is actually in, and nothing else.

Nothing is "lost" at `off`: the roster, the ladder, dials, wipeboard, RIREKI, Koshi — the
edge management that is not about git — is untouched by this leaf. What `off` removes is
exactly the git topology, and only that.

### Shadow SOPs — the developer's own discipline, without editing anything shipped

Everything the brief teaches at either setting is a catalog entry, and catalogs shadow
(`docs/shadowing.md`, the DAIKUSAN law): *put a file with the same name in your catalog
directory and it wins*, entry by `## name`, `hidden: yes` to remove one. So a developer
who wants a different arrangement does not switch Ronin off; they shadow the words:

- their own `github.md` entry — *reviewed mode is a PR per feature branch, squash on
  merge* — replaces the stock one whole, and every session on that install is born
  reading it;
- a `hidden: yes` tombstone on `desk-open` / `hand-in` if they never want those actions
  offered; `control_surface: off` if they never want the topology at all;
- a shadowed `DESK_CONTRACT.md` if they keep desks but change what a hand-in means to
  their team (e.g. "hand in only at a green local test run").

Every entry carries `origin: stock | user`, so *what am I actually running* is always
answerable, and an upgrade never overwrites their words.

### Where a person finds this

- **⚙ (the Admin Desk)**: the `control_surface` leaf, with its teaching text: *on: sessions
  that change code work at desks and hand in; off: sessions work in the repository's
  declared checkout as its own instructions say. Change the words either way in
  Customize.* — a link, not a paragraph.
- **Customize (`#/customize`)**: the control-surface entries listed with their origin, and
  *shadow this* as the affordance on each — the guided-agent path for v1 (an agent writes
  the user entry from the stock one), direct edit later.
- **Atarashi / first run**: one question — *do your coding sessions work at their own
  desks, or in the checkout?* — writing the install leaf; default `on` for a reviewed
  repository, `off` where every repo is direct.
- **`ronin-doctor`**: prints the resolved answer per repository and where it came from.

### What this replaces

`RONIN_DESKS` stays as the operator override during cutover and then as an escape hatch
(`off` on a box where the leaf says `on`, for one launch). The all-or-nothing answers —
"declare the repo direct" or "open a plain terminal" — remain true but are the coarse
ends of a dial that now has an install, team and session position, and words a person can
replace without leaving the product.
